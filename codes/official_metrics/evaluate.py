...
if __name__ == "__main__":
    ...
    if osp.exists(Vid4_SR_dir):
        Vid4_GT_lst = [osp.join(Vid4_GT_dir, vid) for vid in Vid4_vids]
        Vid4_SR_lst = [osp.join(Vid4_SR_dir, vid) for vid in Vid4_vids]
        os.system(
            "python codes/official_metrics/metrics.py --output {} --results {} --targets {}".format(
    ...
    if osp.exists(ToS3_SR_dir):
        ToS3_GT_lst = [osp.join(ToS3_GT_dir, vid) for vid in ToS3_vids]
        ToS3_SR_lst = [osp.join(ToS3_SR_dir, vid) for vid in ToS3_vids]
        os.system(
            "python codes/official_metrics/metrics.py --output {} --results {} --targets {}".format(
